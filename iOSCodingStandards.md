# EPAM Mobile Competency Center Objective-C Style Guide
###### Revision 1.0

This style guide describes the coding conventions suggested by EPAM Mobile Competency Center for any Objective-C based project developed by EPAM.
...
This document might be used in the next two ways:
...
Some rules in this style guide have reasoning to explain why we need a rule and why is it defined this way. But there are a lot of rules that are contractual by their nature, we just need to define these rules to keep our code consistent. The authors of this document are more concerned whether you have some coding guidelines on your project, than whether you use this particular document as your coding guidelines.
...
6. [Methods](#6methods)
    ...
    2. [Autorelease Pool Blocks](#62autorelease-pool-blocks)
    3. [Conditionals](#63conditionals)
        1. [Booleans](#631booleans)
        2. [Ternary Operator](#632ternary-operator)
    4. [Logging](#64logging)
    5. [Error Handling](#65error-handling)
    6. [Assertion](#66assertion)
...
3. Project prefix should be defined and always be used for class names and constants. 
**For example:**
`SF` prefix stands for "Spark Framework".
...
3. It is recommended to turn on "Documentation Comments" setting in your Xcode project settings, this will warn when the documentation’s variable names or return types don’t match the method signature.
...
3. It is recommended to avoid any modifications in 3rd party code and keep it intact. If you absolutely need to modify some 3rd party code then add a comment to explain your changes. Comply with existing formatting in 3rd party code when you modify it.
...
2. If you add a category to a class without project prefix then the category name should have project prefix. If the class name already has project prefix the category name should not have project prefix. ***For example*** `NSObject+SFAttributes`, but `SFServiceProvider+Logging`.
3. If you add a category to a class without project prefix then the category methods should have project prefix with underscore like `SF_`  to avoid method name collisions.
...
9. Prefer to use `instancetype` as method return type instead of generic `id` whenever possible, that is whenever a method returns an instance of its own class. Here is a good article that explains [why you should use instancetype instead of id](http://tewha.net/2013/02/why-you-should-use-instancetype-instead-of-id/).
10. Method length should be no longer than 40 lines. It is preferable to keep method length below 30 lines. If a method becomes very long it is hard to understand. Therefore long methods should usually be refactored into several individual methods that focus on a specific task.
11. Do not declare private methods in class extension (anonymous category) or anywhere else. Only provide private method definition without any separate declaration.
...
2. `init` methods should be placed at the top of an implementation file before any other method implementation.
...
### 6.2.Autorelease Pool Blocks

1. Follow the official guide from Apple about [Using Autorelease Pool Blocks](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/MemoryMgmt/Articles/mmAutoreleasePools.html).
...
### 6.4.Logging
1. If there is a logging solution integrated an appropriate logging level (`info`, `debug`, `warning`, `error`) should be chosen to reflect the severity of the log message.
2. Specify a distinct log message type appropriate for your component (for instance `SparkCore`), so that your messages can be easily filtered out from a user's logs.
...
1. Dot-notation cannot be used with variables types as `id`, so use bracket notation in this case.
2. Dot-notation should be used for accessing and mutating properties. Bracket notation is preferred in all other instances.
...
2. C function declarations in header files should be prefixed with a project prefix
**For example:**
...
3. [Google Objective-C Style Guide](http://google-styleguide.googlecode.com/svn/trunk/Objective-Cguide.xml)
